...
autoBootstrap() {
  ...
  for package in $pkgs; do
    ...
    package_config="$(jq -r ".packages.\"$package\"" release-please-config.json)"

    if [[ "$package_config" == "null" ]]; then
      info "Adding $package to release-please-config.json"
      jqm ".packages.\"$package\" = \"$package_version\"" release-please-config.json
    fi
  done
